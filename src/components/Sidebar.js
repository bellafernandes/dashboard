    ...
    >
      <div className="flex-shrink-0 overflow-hidden p-2">
        <div className="flex items-center h-full sm:justify-center xl:justify-start p-2 sidebar-separator-top">
          ...
          <div className="block sm:hidden xl:block ml-2 font-bold text-xl text-zinc-900/90 dark:text-gray-300">
            Logo
          </div>
...
function MenuItem({ item: { id, title, notifications }, onClick, selected }) {
  return (
    <div
      className={clsx(
        "w-full mt-6 mb-10 flex items-center px-3 sm:px-0 xl:px-3 justify-start sm:justify-center xl:justify-start sm:mt-6 xl:mt-3 cursor-pointer text-zinc-900/90 dark:text-gray-300",
        selected === id ? "sidebar-item-selected" : "sidebar-item"
      )}
      onClick={() => onClick(id)}
    >
      <SidebarIcons id={id} />
      <div className="block sm:hidden xl:block ml-2 text-gray-700 hover:text-gray-500/90 dark:text-gray-400 dark:hover:text-gray-200 transition duration-200">{title}</div>
      <div className="block sm:hidden xl:block flex-grow" />
      {notifications && (
        <div className="flex sm:hidden xl:flex bg-purple-500 w-5 h-5 flex items-center justify-center rounded-full mr-2">
...
function SidebarIcons({ id }) {
  ...
  return (
    <svg
      className="w-8 h-8 xl:w-5 xl:h-5 text-zinc-900/90 dark:text-gray-300"
      viewBox="0 0 24 24"
      fill="currentColor"
